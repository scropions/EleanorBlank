...
inline char const* glErrorString(const GLenum err) noexcept
{
    switch (err)
    {
    // OpenGL 2.0+ Errors:
...
GPUDriverGL::GPUDriverGL(GLFWwindow* window, Config* config) : _window(window),
    _config(config)
{
}
                               ...
                               const ultralight::GPUState& state)
{
    ...
    GeometryEntry& geometry = geometry_map[geometry_id];
    SelectProgram((ultralight::ShaderType)state.shader_type);
...
void GPUDriverGL::DrawCommandList()
{
    ...
    for (auto i = render_buffer_map.begin(); i != render_buffer_map.end(); ++i)
    {
        ...
        if (rbuf.bitmap && rbuf.needs_update)
        {
            ...
            glBindFramebuffer(GL_FRAMEBUFFER, i->second.fbo_map.at(0).fbo_id);
            CHECK_GL();
...
void GPUDriverGL::LoadProgram(ultralight::ShaderType type)
{
    GLenum ErrorCheckValue = glGetError();
    ProgramEntry prog;
...
void GPUDriverGL::SelectProgram(ultralight::ShaderType type)
{
...
void GPUDriverGL::UpdateUniforms(const ultralight::GPUState& state)
{
    bool flip_y = state.render_buffer_id != 0;
    ultralight::Matrix model_view_projection = ApplyProjection(state.transform, (float)state.viewport_width,
...
ultralight::Matrix GPUDriverGL::ApplyProjection(const ultralight::Matrix4x4& transform, float screen_width,
...
void GPUDriverGL::CreateFBOTexture(uint32_t texture_id, ultralight::RefPtr<ultralight::Bitmap> bitmap)
{
    CHECK_GL();
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
void GPUDriverGL::CreateFBOIfNeededForActiveContext(uint32_t render_buffer_id)
{
    ...
    if (result != GL_FRAMEBUFFER_COMPLETE)
        FATAL("Error creating MSAA FBO, this usually fails if your DPI scale is invalid or View dimensions are massive: " <<
              result);
    CHECK_GL();
}
...
uint32_t GPUDriverGL::NextTextureId()
{
    return next_texture_id_++;
}

uint32_t GPUDriverGL::NextRenderBufferId()
{
    return next_render_buffer_id_++;
}

uint32_t GPUDriverGL::NextGeometryId()
{
    return next_geometry_id_++;
}

bool GPUDriverGL::HasCommandsPending()
{
    return !command_list_.empty();
}

void GPUDriverGL::BeginSynchronize() {}

void GPUDriverGL::EndSynchronize() {}

void GPUDriverGL::UpdateCommandList(const ultralight::CommandList& list)
{
    if (list.size)
    {
